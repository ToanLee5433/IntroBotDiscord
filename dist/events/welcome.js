...
exports.registerWelcomeEvent = registerWelcomeEvent;
 ...
 */
function registerWelcomeEvent(client) {
    // 1. LẮNG NGHE THÀNH VIÊN MỚI GIA NHẬP GUILD
    client.on('guildMemberAdd', async (member) => {
        try {
            await (0, utils_1.sleep)(1000); // Chờ 1 giây để Discord load profile ổn định
            ...
            if (guild.id === '1209955080649052160') {
                ...
                const ch1 = guild.channels.cache.find(ch => ch.isTextBased() && !ch.isThread() && ch.viewable &&
                    ch.name === 'chào-mừng-và-nội-quy' &&
                    ch.permissionsFor(me)?.has(discord_js_1.PermissionFlagsBits.SendMessages));
                const ch2 = guild.channels.cache.find(ch => ch.isTextBased() && !ch.isThread() && ch.viewable &&
                    ch.name === 'chung' &&
                    ch.permissionsFor(me)?.has(discord_js_1.PermissionFlagsBits.SendMessages));
                if (ch1)
                    targetChannels.push(ch1);
                if (ch2)
                    targetChannels.push(ch2);
            }
            ...
            if (targetChannels.length === 0) {
                ...
                const targetChannel = guild.channels.cache.find(ch => {
                    if (ch.isTextBased() && !ch.isThread() && ch.viewable) {
                        ...
                        if (perms && perms.has(discord_js_1.PermissionFlagsBits.SendMessages)) {
                ...
                if (!welcomeChannel) {
                    welcomeChannel = guild.channels.cache.find(ch => {
                        if (ch.isTextBased() && !ch.isThread() && ch.viewable) {
                            const perms = ch.permissionsFor(me);
                            return perms !== null && perms.has(discord_js_1.PermissionFlagsBits.SendMessages);
                        }
                        return false;
                    });
                }
            ...
            let claimButtonStyle = discord_js_1.ButtonStyle.Danger;
            ...
            if (isRoyal) {
                ...
                const admins = guild.members.cache.filter(m => !m.user.bot && m.permissions.has(discord_js_1.PermissionFlagsBits.Administrator));
                ...
                welcomeEmbed = new discord_js_1.EmbedBuilder()
                    .setTitle(`👑 CUNG NGHÊNH NỮ HOÀNG TỐI CAO - THÁI HẬU SỜI BẠC GIA NHẬP! 👑`)
                    .setDescription(`🙇‍♂️ Kính cẩn nghiêng mình cúi đầu cung nghênh **Nữ Hoàng ${freshMember.user.username}** (<@${freshMember.id}>) đã hạ giá lâm phàm, ghé thăm vương quốc sới bạc hoàng gia **${guild.name}** (thành viên thứ **${guild.memberCount}** của server)!\n\n` +
                ...
                claimButtonStyle = discord_js_1.ButtonStyle.Success; // Màu xanh lá hoàng tộc
            }
            else {
            ...
            const guideButton = new discord_js_1.ButtonBuilder()
                .setCustomId(`welcome_guide`)
                .setLabel("📖 Hướng Dẫn Nhanh")
                .setStyle(discord_js_1.ButtonStyle.Primary);
            const row = new discord_js_1.ActionRowBuilder().addComponents(claimButton, guideButton);
            ...
            for (const welcomeChannel of targetChannels) {
                await welcomeChannel.send({
                    ...
                    components: [row],
                    files: files
                }).catch((err) => console.error(`Lỗi gửi chào mừng tại kênh #${welcomeChannel.name}:`, err));
            }
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if (customId === 'welcome_guide') {
            const guideEmbed = new discord_js_1.EmbedBuilder()
                ...
                .setColor(0x3498DB)
                .addFields({ name: "1. 💵 Điểm Danh Hàng Ngày", value: "Tag `@BotToan diem danh` hoặc `@BotToan daily` để nhận vốn từ 10k - 50k mỗi ngày.", inline: false }, { name: "2. 🏦 Kiểm Tra Ví & Tài Sản", value: "Tag `@BotToan vi` hoặc `@BotToan tai san` để xem số dư và nợ nần đang gánh.", inline: false }, { name: "3. 🎲 Casino Đỏ Đen (Tương tác nút)", value: "Tag `@BotToan tai xiu` hoặc `@BotToan blackjack` để bắt đầu sới bạc gỡ nợ.", inline: false }, { name: "4. 🤖 Trò Chuyện Với AI Mỏ Hỗn", value: "Tag `@BotToan [nội dung]` để tâm sự mỏng cùng Gemini AI.", inline: false }, { name: "5. 📖 Xem Cẩm Nang Chi Tiết", value: "Tag `@BotToan help` để mở cẩm nang đầy đủ tất cả các tính năng.", inline: false })
        ...
        if (!customId.startsWith('welcome_claim_'))
            return;
        ...
        try {
            const dbResult = await (0, database_1.claimWelcomeGift)(clickerId);
            if (!dbResult.success) {
            ...
            const guideButton = new discord_js_1.ButtonBuilder()
                ...
                .setLabel("📖 Hướng Dẫn Nhanh")
                .setStyle(discord_js_1.ButtonStyle.Primary);
            const disabledRow = new discord_js_1.ActionRowBuilder().addComponents(disabledButton, guideButton);
