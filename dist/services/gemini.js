...
exports.checkImageQuota = checkImageQuota;
exports.chatWithGemini = chatWithGemini;
exports.getMatchmakingFortune = getMatchmakingFortune;
exports.getTarotReading = getTarotReading;
exports.getAuraReading = getAuraReading;
exports.processAnonymousLetter = processAnonymousLetter;
exports.getMoodAdvice = getMoodAdvice;
exports.getOverthinkAnalysis = getOverthinkAnalysis;
exports.getShoppingVerdict = getShoppingVerdict;
exports.getRealGuReading = getRealGuReading;
exports.getGuMatchReading = getGuMatchReading;
exports.rateValorantTeam = rateValorantTeam;
exports.generateGamingCourtVerdict = generateGamingCourtVerdict;
exports.getWCPrediction = getWCPrediction;
exports.analyzeImageWithGemini = analyzeImageWithGemini;
exports.generateImageWithImagen = generateImageWithImagen;
exports.editImageWithImagen = editImageWithImagen;
const generative_ai_1 = require("@google/generative-ai");
const genai_1 = require("@google/genai");
const config_1 = require("../config");
if (!config_1.GEMINI_KEY) {
    console.error("[GEMINI LỖI] Thiếu GEMINI_API_KEY trong biến môi trường!");
}
const genAI = new generative_ai_1.GoogleGenerativeAI(config_1.GEMINI_KEY || '');
// SDK mới dùng cho Imagen 4 Ultra
const genAINew = new genai_1.GoogleGenAI({ apiKey: config_1.GEMINI_KEY || '' });
...
function checkImageQuota(userId) {
 ...
 */
async function chatWithGemini(userId, userQuestion) {
 ...
 */
async function getMatchmakingFortune(prompt) {
    if (!config_1.GEMINI_KEY) {
 ...
 */
async function getAuraReading(color, userName, targetColor, targetName) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function processAnonymousLetter(content, tone) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function getMoodAdvice(mood, userName, tarotCard) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function getOverthinkAnalysis(situation) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function getShoppingVerdict(item, price, verdict, regretScore) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
    ...
    const result = await model.generateContent(`Món đồ: "${item}"${price ? ` | Giá: ${price}` : ''}. Chỉ số hối hận dự kiến: ${regretScore}%. Viết lý do phán quyết ${verdict}.`);
    return result.response.text();
}
/**
 * Đọc vị gu người yêu thực tế dựa trên mã trả lời trắc nghiệm (REAL_GU_PROMPT)
 */
async function getRealGuReading(answers, username) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function getGuMatchReading(myName, myGuSummary, targetName, targetProfileText, targetGuSummary, score) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function rateValorantTeam(teamList) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function generateGamingCourtVerdict(username, gameName, duration, pronoun) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function getWCPrediction(teamA, teamB, pronoun) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
    const model = genAI.getGenerativeModel({
        model: "gemini-3.1-flash-lite",
 ...
 */
async function analyzeImageWithGemini(imageUrl, mimeType, userPrompt) {
    if (!config_1.GEMINI_KEY)
        throw new Error("Missing Gemini key");
 ...
 */
async function refineImagePrompt(vietnamesePrompt) {
    if (!config_1.GEMINI_KEY)
 ...
 */
async function generateImageWithImagen(userId, prompt) {
    ...
    if (!imageBytes) {
        ...
        try {
            ...
            if (!pollResponse.ok)
                throw new Error(`Pollinations AI error: HTTP ${pollResponse.status}`);
 ...
 */
async function editImageWithImagen(userId, imageBase64, mimeType, instruction) {
    const quota = checkImageQuota(userId);
    if (!quota.allowed) {
        throw new Error(`QUOTA_EXCEEDED:${quota.used}:${quota.limit}`);
    }
    ...
    for (const modelId of candidateModels) {
        try {
            ...
            imageBytes = editResponse?.generatedImages?.[0]?.image?.imageBytes;
            if (imageBytes) {
    ...
    if (!imageBytes) {
        ...
        for (const modelId of candidateModels) {
            try {
                ...
                imageBytes = textOnlyResponse?.generatedImages?.[0]?.image?.imageBytes;
                if (imageBytes) {
