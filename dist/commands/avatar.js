...
exports.getAvatarPage = getAvatarPage;
exports.handleAvatarCommand = handleAvatarCommand;
exports.registerAvatarCollector = registerAvatarCollector;
const discord_js_1 = require("discord.js");
/**
 * Trả về trang hiển thị Avatar của thành viên
 */
async function getAvatarPage(targetUser, guildMember, type) {
 ...
 */
async function handleAvatarCommand(message, cleanInput) {
    ...
    if (potentialArg) {
        ...
        if (mentionMatch) {
            ...
            if (idMatch) {
                ...
                const foundMember = message.guild?.members.cache.find(m => m.displayName.toLowerCase().includes(nameQuery) ||
 ...
 */
function registerAvatarCollector(client) {
    client.on('interactionCreate', async (interaction) => {
        const id = interaction.customId;
        if (!id || !id.startsWith('avt_'))
            return;
