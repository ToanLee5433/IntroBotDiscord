...
exports.handleWCPrediction = handleWCPrediction;
exports.playWCPenalty = playWCPenalty;
exports.handleWCCommand = handleWCCommand;
exports.getWCHomePage = getWCHomePage;
exports.getWCMatchDetailPage = getWCMatchDetailPage;
exports.getWCAdminHomePage = getWCAdminHomePage;
exports.getWCAdminConsole = getWCAdminConsole;
exports.getWCSettleConsole = getWCSettleConsole;
exports.registerWorldCupCollector = registerWorldCupCollector;
 ...
 */
async function handleWCPrediction(message, rawInput) {
    ...
    const profile = await (0, database_1.getProfile)(message.author.id).catch(() => null);
    let pronoun = "thí chủ";
    ...
    try {
        const prediction = await (0, gemini_1.getWCPrediction)(teamA, teamB, pronoun);
 ...
 */
async function playWCPenalty(message, rawInput) {
    if (!message.guild) {
    ...
    let betAmount = arg ? (0, utils_1.parseMoneyInput)(arg) : 20; // mặc định 20k
    ...
    if (balance < betAmount) {
        await message.reply(`❌ **Nghèo mà đòi đá phạt đền!** Số dư của bạn chỉ còn **${(0, utils_1.formatMoney)(balance)}**, không đủ để đặt cược **${(0, utils_1.formatMoney)(betAmount)}**!`).catch(() => { });
        return;
    }
    // Khấu trừ tiền cược trước để đảm bảo chống cheat
    await (0, database_1.updateBalance)(userId, balance - betAmount);
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription(`Đối mặt với thủ môn huyền thoại **BotToan**. Mày có cơ hội sút phạt đền kiếm tiền thưởng!\n` +
        `💰 **Tiền cược:** **${(0, utils_1.formatMoney)(betAmount)}**\n` +
        `👉 Hãy click chọn hướng sút bên dưới trong 30 giây để thực hiện cú sút!${voiceMessage}`)
        .setColor(0x2ECC71)
        .setThumbnail(message.author.displayAvatarURL());
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('pen_left').setLabel('⚽ Sút Trái').setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId('pen_center').setLabel('⚽ Sút Giữa').setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId('pen_right').setLabel('⚽ Sút Phải').setStyle(discord_js_1.ButtonStyle.Danger));
    ...
    collector.on('collect', async (interaction) => {
        ...
        let playerChoice = "";
        if (buttonId === 'pen_left')
            playerChoice = "Trái";
        else if (buttonId === 'pen_center')
            playerChoice = "Giữa";
        else
            playerChoice = "Phải";
        // Vô hiệu hóa nút bấm ngay lập tức
        const disabledRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('pen_left').setLabel('⚽ Sút Trái').setStyle(discord_js_1.ButtonStyle.Primary).setDisabled(true), new discord_js_1.ButtonBuilder().setCustomId('pen_center').setLabel('⚽ Sút Giữa').setStyle(discord_js_1.ButtonStyle.Success).setDisabled(true), new discord_js_1.ButtonBuilder().setCustomId('pen_right').setLabel('⚽ Sút Phải').setStyle(discord_js_1.ButtonStyle.Danger).setDisabled(true));
        ...
        if (isHitPost) {
            ...
            const hitPostMessages = [
                `💥 **Cú sút đi vào lòng đất!** Bạn sút hướng **${playerChoice}**, bóng bay cực căng NHƯNG đập trúng cột dọc dội ngược lại đập thẳng vào mặt bạn! 🤕 Cột dọc cứu thua cho BotToan, bay màu mất sạch **${(0, utils_1.formatMoney)(betAmount)}** cược nhé cưng!`,
                `💥 **Ối dồi ôi!** Cú sút hướng **${playerChoice}** khiến thủ môn BotToan đứng hình chịu chết... NHƯNG bóng đập trúng xà ngang nảy ra ngoài! 😭 Nhân phẩm quá thấp, cột dọc xà ngang gánh còng lưng BotToan rồi. Tiếc nuối mất đi **${(0, utils_1.formatMoney)(betAmount)}**!`,
                `💥 **Trắng tay vì cột dọc!** Bạn sút về hướng **${playerChoice}**, bóng bay trúng cột dọc rồi bay thẳng ra chuồng gà! 🐔 *BotToan khịa:* "Đã nghèo còn gặp cột dọc gánh team. Thôi nạp thêm tiền đi em ơi, mất sạch **${(0, utils_1.formatMoney)(betAmount)}** rồi!"`
            ];
            ...
            const savedMessages = [
                `🧤 **Bị tóm gọn quả bóng!** Bạn sút hướng **${playerChoice}**, thủ môn BotToan bay người nhẹ nhàng ôm gọn trái bóng như ôm người yêu cũ! 🤡 *Khịa:* "Đá nhẹ thế này thì về quê chăn vịt đi em ơi!", mất sạch **${(0, utils_1.formatMoney)(betAmount)}**!`,
                `🧤 **Bắt bài quá dễ!** Bạn sút hướng **${playerChoice}**, BotToan chỉ cần nhấc nhẹ cái chân là cản phá thành công! 🦵 *Khịa:* "Đọc vị như đọc sách giáo khoa cấp 1. Nộp **${(0, utils_1.formatMoney)(betAmount)}** cống nạp cho nhà cái lẹ đi cưng!"`,
                `🧤 **Hết cứu!** Cú sút hướng **${playerChoice}** quá hiền lành, BotToan bay người cản phá xuất thần! 🧤 *Khịa:* "Sút thế này thì đến thủ môn mù cũng đỡ được. Mất trắng **${(0, utils_1.formatMoney)(betAmount)}** cược nhé bạn yêu!"`
            ];
            ...
            const currentBal = await (0, database_1.getBalance)(userId);
            await (0, database_1.updateBalance)(userId, currentBal + (betAmount * 2));
            const goalMessages = [
                `🎉 **VÀOOOOOO! Đỉnh nóc kịch trần!** Bạn sút sang **${playerChoice}**, BotToan bay người ngơ ngác sang **${botChoice}** như kẻ mất sổ gạo! 🤡 Húp trọn **${(0, utils_1.formatMoney)(betAmount * 2)}** cược, uy tín quá em ơi!`,
                `🎉 **Rung lưới ngọt nước!** Cú sút hiểm hóc hướng **${playerChoice}** xé toạc mành lưới trong sự bất lực của BotToan! 🔥 Hốt ngay **${(0, utils_1.formatMoney)(betAmount * 2)}** về ví, hôm nay tổ độ rồi!`,
                `🎉 **Quá đẳng cấp!** Bạn sút hướng **${playerChoice}** đánh lừa hoàn toàn thủ môn BotToan (bay hướng **${botChoice}**)! ⚽ *Tấu hài:* "Bóng bay vào lưới đẹp như tranh vẽ, BotToan chỉ biết đứng nhìn khóc thét!" Thắng cược nhận **${(0, utils_1.formatMoney)(betAmount * 2)}**!`
            ];
        ...
        const resultEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(color)
            .addFields({ name: "🎯 Hướng sút của bạn", value: `**${playerChoice}**`, inline: true }, { name: "🧤 Thủ môn BotToan bay", value: `**${botChoice}**`, inline: true })
            .setThumbnail(message.author.displayAvatarURL())
    ...
    collector.on('end', async (collected, reason) => {
        if (reason === 'time') {
            ...
            const timeoutRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('pen_left').setLabel('Sút Trái').setStyle(discord_js_1.ButtonStyle.Primary).setDisabled(true), new discord_js_1.ButtonBuilder().setCustomId('pen_center').setLabel('Sút Giữa').setStyle(discord_js_1.ButtonStyle.Success).setDisabled(true), new discord_js_1.ButtonBuilder().setCustomId('pen_right').setLabel('Sút Phải').setStyle(discord_js_1.ButtonStyle.Danger).setDisabled(true));
            const timeoutEmbed = new discord_js_1.EmbedBuilder()
                .setTitle("⏰ QUÁ HẠN SÚT PHẠT ĐỀN")
                .setDescription(`⏰ Hết giờ! Bạn đã suy nghĩ quá lâu (30 giây) để thực hiện cú sút.\nThủ môn BotToan đã nhặt bóng đi về và tịch thu **${(0, utils_1.formatMoney)(betAmount)}** tiền cược của bạn làm phí sân cỏ.`)
                .setColor(0x7F8C8D);
 ...
 */
async function handleWCCommand(message, rawInput) {
    ...
    if (subCommand === 'bat' || subCommand === 'bet') {
        ...
        const match = await (0, database_1.getWCMatch)(matchId);
        if (!match) {
        ...
        if (upperChoice === 'A') {
            ...
            const cleanChoice = (0, utils_1.removeAccents)(choice).toLowerCase().replace(/\s+/g, '');
            const cleanTeamA = (0, utils_1.removeAccents)(match.teamA).toLowerCase().replace(/\s+/g, '');
            const cleanTeamB = (0, utils_1.removeAccents)(match.teamB).toLowerCase().replace(/\s+/g, '');
        ...
        const amount = (0, utils_1.parseMoneyInput)(moneyArg);
        if (amount === null || isNaN(amount) || amount <= 0) {
            await message.reply("❌ **Số tiền đặt cược không hợp lệ!** Ví dụ cược: `50k`, `100k`, `1.5tr`...").catch(() => { });
            return;
        }
        const res = await (0, database_1.placeWCBet)(message.author.id, matchId, finalChoice, amount);
    ...
    if (subCommand === 'setwc') {
        ...
        const success = await (0, database_1.addWCMatch)(matchId, teamA, teamB, odds);
        if (success) {
    ...
    if (subCommand === 'lockwc') {
        ...
        const success = await (0, database_1.lockWCMatch)(matchId);
        if (success) {
    ...
    if (subCommand === 'chungwc' || subCommand === 'chung') {
        ...
        const result = await (0, database_1.settleWCMatch)(matchId, winner);
    ...
    if (subCommand === 'editwc') {
        ...
        const teamA = vsParts[0].trim();
        const teamB = vsParts[1].trim();
        const success = await (0, database_1.updateWCMatch)(matchId, teamA, teamB, odds);
        if (success) {
    ...
    if (subCommand === 'delwc' || subCommand === 'xoawc') {
        ...
        const result = await (0, database_1.deleteWCMatch)(matchId);
        if (result.success) {
    ...
    if (subCommand === 'qlwc' || subCommand === 'listwc') {
        ...
        const matches = await (0, database_1.getAllWCMatches)();
        if (matches.length === 0) {
        ...
        for (const m of matches) {
            let statusText = "";
            if (m.status === 'open')
                statusText = "🟢 Đang mở đặt cược";
            else if (m.status === 'locked')
                statusText = "🔒 Đã khóa đặt cược";
            else if (m.status === 'ended')
                statusText = `🏁 Đã kết thúc (Thắng kèo: **${m.winner === 'HoaKeo' ? 'Hòa Kèo' : (m.winner === 'A' ? m.teamA : m.teamB)}**)`;
            embed.addFields({
 ...
 */
async function getWCHomePage(authorId) {
    const matches = await (0, database_1.getActiveWCMatches)();
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle("🏟️ BẢNG TỶ LỆ CÁ CƯỢC WORLD CUP 2026")
        .setDescription(`Chào mừng đến với sòng cá cược World Cup ảo của BotToan!\n` +
    ...
    if (matches.length === 0) {
        ...
        if (openMatches.length > 0) {
            const selectMenu = new discord_js_1.StringSelectMenuBuilder()
                ...
                .addOptions(openMatches.map(m => new discord_js_1.StringSelectMenuOptionBuilder()
 ...
 */
async function getWCMatchDetailPage(matchId, authorId) {
    const match = await (0, database_1.getWCMatch)(matchId);
    if (!match) {
        const errEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(0xE74C3C);
        const backBtn = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription(`• **Mã trận:** \`${match.matchId}\`\n` +
    ...
    if (match.status === 'open') {
        const betRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
 ...
 */
async function getWCAdminHomePage(authorId) {
    const matches = await (0, database_1.getAllWCMatches)();
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle("⚙️ BẢNG QUẢN LÝ TRẬN ĐẤU WORLD CUP 2026")
    ...
    if (matches.length === 0) {
        ...
        for (const m of matches) {
            ...
            if (m.status === 'open')
                ...
                statusText = `🏁 Đã kết thúc (Thắng: **${m.winner === 'HoaKeo' ? 'Hòa Kèo' : (m.winner === 'A' ? m.teamA : m.teamB)}**)`;
            embed.addFields({
                name: `⚽ Trận \`${m.matchId}\`: ${m.teamA} vs ${m.teamB}`,
                value: `• **Trạng thái:** ${statusText}\n• **Kèo chấp:** *${m.odds}*`,
                inline: false
        ...
        const selectMenu = new discord_js_1.StringSelectMenuBuilder()
            ...
            .addOptions(matches.map(m => new discord_js_1.StringSelectMenuOptionBuilder()
            .setLabel(`Trận ${m.matchId.toUpperCase()}: ${m.teamA} vs ${m.teamB}`)
 ...
 */
async function getWCAdminConsole(matchId, authorId) {
    const match = await (0, database_1.getWCMatch)(matchId);
    if (!match) {
        const errEmbed = new discord_js_1.EmbedBuilder()
            .setTitle("❌ Trận đấu không tồn tại")
    ...
    if (match.status === 'open')
        ...
        statusText = `🏁 Đã kết thúc (Thắng: **${match.winner === 'HoaKeo' ? 'Hòa Kèo' : (match.winner === 'A' ? match.teamA : match.teamB)}**)`;
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle(`⚙️ CONSOLE QUẢN TRỊ: TRẬN \`${match.matchId.toUpperCase()}\``)
        .setDescription(`**Thông tin hiện tại:**\n` +
 ...
 */
async function getWCSettleConsole(matchId, authorId, disabled = false) {
    const match = await (0, database_1.getWCMatch)(matchId);
    if (!match) {
        const errEmbed = new discord_js_1.EmbedBuilder()
            .setTitle("❌ Trận đấu không tồn tại")
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription(`Vui lòng chọn kết quả thắng cuộc của trận đấu:\n` +
        `⚽ **Cặp đấu:** **${match.teamA}** vs **${match.teamB}**\n` +
        `⭐ **Kèo chấp:** *${match.odds}*\n\n` +
        ...
        .setFooter({ text: "BotToan System Manager" })
        .setTimestamp();
    const settleRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
 ...
 */
function registerWorldCupCollector(client) {
    client.on('interactionCreate', async (interaction) => {
        const id = interaction.customId;
        if (!id || !id.startsWith('wc_'))
            return;
        ...
        try {
            ...
            if (interaction.isButton()) {
                ...
                if (id.startsWith('wc_btn_bet_')) {
                    ...
                    const amountInput = new discord_js_1.TextInputBuilder()
                        ...
                        .setRequired(true);
                    const firstActionRow = new discord_js_1.ActionRowBuilder().addComponents(amountInput);
                ...
                if (id.startsWith('wc_btn_history_personal_')) {
                    ...
                    const userBets = await (0, database_1.getUserWCBets)(interaction.user.id);
                    ...
                    for (let i = 0; i < Math.min(userBets.length, 10); i++) {
                        ...
                        if (bet.settled) {
                            if (m && m.winner === 'HoaKeo') {
                                statusText = `🟡 **Hòa kèo (Hoàn lại ${(0, utils_1.formatMoney)(bet.amount)})**`;
                            }
                            else if (m && bet.team === m.winner) {
                                statusText = `🟢 **Thắng (+${(0, utils_1.formatMoney)(bet.amount * 2)})**`;
                            }
                            else {
                                statusText = `🔴 **Thua (-${(0, utils_1.formatMoney)(bet.amount)})**`;
                            }
                        }
                        else {
                        ...
                        embed.addFields({
                            ...
                            value: `• **Lựa chọn:** Cửa ${bet.team} (${choiceName})\n• **Số tiền:** **${(0, utils_1.formatMoney)(bet.amount)}**\n• **Kết quả:** ${statusText}`,
                            inline: false
                        });
                    }
                ...
                if (id.startsWith('wc_btn_active_wagers_')) {
                    await interaction.deferReply({ ephemeral: true }).catch(() => { });
                    const activeBets = await (0, database_1.getActiveWCBets)();
                    ...
                    for (const [matchKey, wagers] of betsByMatch.entries()) {
                        if (embedFieldsCount >= 10)
                            break;
                        let wagersText = "";
                        for (const w of wagers) {
                            const m = w.match;
                            const choiceName = m ? (w.bet.team === 'A' ? m.teamA : m.teamB) : w.bet.team;
                            wagersText += `- <@${w.bet.userId}> cược **${(0, utils_1.formatMoney)(w.bet.amount)}** vào cửa **${choiceName}**\n`;
                        }
                        embed.addFields({
                            name: `⚽ ${matchKey}`,
                ...
                if (id.startsWith('wc_adm_settle_')) {
                    ...
                    const res = await (0, database_1.settleWCMatch)(matchId, choice === 'HOAKEO' ? 'HoaKeo' : choice);
                    if (res.success) {
                ...
                if (id.startsWith('wc_admin_edit_')) {
                    ...
                    const teamAInput = new discord_js_1.TextInputBuilder()
                        .setCustomId('edit_team_a')
                        .setLabel("Đội A (Ví dụ: Tây Ban Nha)")
                        .setStyle(discord_js_1.TextInputStyle.Short)
                        .setValue(match.teamA)
                        .setRequired(true);
                    const teamBInput = new discord_js_1.TextInputBuilder()
                    ...
                    const oddsInput = new discord_js_1.TextInputBuilder()
                        ...
                        .setValue(match.odds)
                        .setRequired(true);
                    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(teamAInput), new discord_js_1.ActionRowBuilder().addComponents(teamBInput), new discord_js_1.ActionRowBuilder().addComponents(oddsInput));
                    await interaction.showModal(modal).catch(() => { });
                    return;
                }
            ...
            if (interaction.isModalSubmit()) {
                ...
                if (id.startsWith('wc_modal_bet_')) {
                    ...
                    const amount = (0, utils_1.parseMoneyInput)(betAmountText);
                    if (amount === null || isNaN(amount) || amount <= 0) {
                    ...
                    await interaction.deferReply({ ephemeral: true }).catch(() => { });
                    const res = await (0, database_1.placeWCBet)(interaction.user.id, matchId, team, amount);
