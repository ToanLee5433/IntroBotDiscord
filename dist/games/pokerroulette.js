...
exports.playPokerRoulette = playPokerRoulette;
 ...
 */
async function playPokerRoulette(message, betAmount) {
    ...
    if (debt >= 500) {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`💀 **MÀY ĐANG NỢ KỊCH TRẦN (>= 500K)!**\nHiện tại mày đang nợ ngân hàng BotToan tổng cộng **${(0, utils_1.formatMoney)(debt)}**.\n\nTheo luật **"Nợ là Danh dự"**, mày bị cấm tham gia sòng cờ bạc Poker Roulette khi nợ kịch trần (>= 500k)! Mau gõ \`@BotToan tra no het\` để trả nợ rồi mới được tạo phòng chơi con ạ!`)
            .setColor(0xFF0000)
    ...
    if (betAmount < 10) {
        await message.reply(`❌ Tiền cược tối thiểu để chơi Poker Roulette là **${(0, utils_1.formatMoney)(10)}**!`);
        return;
    }
    ...
    const sessionPlayers = [creatorId];
    let isStarted = false;
    const lobbyRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('pr_join').setLabel(`🤝 Tham gia cược ${(0, utils_1.formatMoney)(betAmount)}`).setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId('pr_start').setLabel('🔫 Bắt đầu bắn').setStyle(discord_js_1.ButtonStyle.Danger));
 ...
 */
async function runGame(lobbyMsg, userIds, betAmount) {
    ...
    // Định dạng bài viết
    const formatCard = (c) => `\`[ ${c.value}${c.suit} ]\``;
    ...
    const getRoundNameVi = () => {
        if (currentRound === 'PRE_FLOP')
            return "🔫 KHỞI ĐỘNG (PRE-FLOP) - SÚNG 1 VIÊN";
        if (currentRound === 'FLOP')
            return "🃏 VÒNG FLOP - LẬT 3 LÁ CHUNG - SÚNG 2 VIÊN";
        if (currentRound === 'TURN')
            return "🃏 VÒNG TURN - LẬT LÁ THỨ 4 - SÚNG 3 VIÊN";
        if (currentRound === 'RIVER')
            return "🃏 VÒNG RIVER - LẬT LÁ CUỐI CÙNG - SÚNG 4 VIÊN";
    ...
    const updateGameEmbed = (activePlayerId = "") => {
        ...
        const playerStatus = playOrder.map((p, idx) => {
            ...
            let status = "";
            if (!p.alive)
                status = "💀 BỊ BẮN CHẾT (ĐANG TRONG TÙ)";
            else if (p.folded)
                status = "🏃 ĐÃ BỎ BÀI (Spectator)";
            else
                status = `🔫 ${p.bullets} viên đạn trong ổ`;
            return `${arrow}**${idx + 1}.** <@${p.userId}>: ${status}`;
        }).join("\n");
        return new discord_js_1.EmbedBuilder()
            .setTitle("🔥 POKER TỬ THẦN (RUSSIAN POKER ROULETTE)")
            .setDescription(`🎲 **Bài chung trên bàn:**\n${comCards}\n\n💰 **Tổng hũ cược:** **${(0, utils_1.formatMoney)(totalPot)}**\n⌛ **Vòng đấu:** \`${roundTitle}\`\n\n👥 **Danh sách đấu sĩ:**\n${playerStatus}\n\n📝 **Nhật ký:**\n${lastLogText}`)
            .setColor(0xE74C3C)
            .setFooter({ text: "Người chơi có 60s để hành động trước khi súng tự cướp cò!" });
    };
    const getButtons = (activePlayer) => {
        const foldLabel = `💥 Bỏ bài (Bóp cò súng ${getRoundBullets()} viên - Tỉ lệ chết ${getFoldDeathChance()})`;
        return new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('poker_call').setLabel(getCallActionLabel()).setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId('poker_fold').setLabel(foldLabel).setStyle(discord_js_1.ButtonStyle.Danger), new discord_js_1.ButtonBuilder().setCustomId('poker_view').setLabel('👁️ Xem 2 lá bài tẩy').setStyle(discord_js_1.ButtonStyle.Primary));
    };
    // Hàm thực hiện bóp cò tự sát
    const pullTrigger = async (player, bullets) => {
        ...
        if (roll < bullets) {
            ...
            try {
                ...
                await (0, database_1.banChat)(player.userId, 180000);
                await (0, utils_1.sendToJail)(lobbyMsg.guild, player.userId, "Bị bắn chết trong sòng Poker Roulette - Áp giải vào Nhà tù");
            }
            catch (err) { }
            return true;
        }
    ...
    const nextTurn = async () => {
        ...
        if (activeCount === 1) {
            ...
            let bal = await (0, database_1.getBalance)(winner.userId);
            bal += totalPot;
            await (0, database_1.updateBalance)(winner.userId, bal);
            const winEmbed = new discord_js_1.EmbedBuilder()
                .setTitle("🏆 TRẬN ĐẤU KẾT THÚC: CHIẾN THẦN ĐỘC TÔN")
                .setDescription(`🎉 Chúc mừng <@${winner.userId}> đã sống sót kiên cường và thắng toàn bộ hũ tiền trị giá **${(0, utils_1.formatMoney)(totalPot)}** do toàn bộ đối thủ đã úp bài hoặc chết!\n\n💰 Ví của mày hiện tại: **${(0, utils_1.formatMoney)(bal)}**`)
                .setColor(0x2ECC71);
        ...
        if (activeCount === 0) {
            const winEmbed = new discord_js_1.EmbedBuilder()
                ...
                .setDescription(`Toàn bộ đấu sĩ đã bỏ bài tử nạn hoặc nổ súng vỡ sọ. Hũ tiền **${(0, utils_1.formatMoney)(totalPot)}** sẽ được xung công quỹ ngân hàng BotToan! Cảm ơn sự cống hiến bằng mạng sống của các con giời!`)
                .setColor(0x000000);
            await lobbyMsg.edit({ embeds: [winEmbed], components: [] }).catch(() => { });
            return;
        }
    ...
    const handleShowdown = async () => {
        ...
        const solvedHands = showdownPlayers.map(p => {
            ...
            const solved = pokersolver_1.Hand.solve(solverCards);
            solved.playerId = p.userId; // Gán ID để truy vết lại
            solved.originalPlayer = p;
            return solved;
        });
        // Tìm bài thắng cuộc
        const winningHands = pokersolver_1.Hand.winners(solvedHands);
        const winningPlayerIds = winningHands.map((w) => w.playerId);
        ...
        for (const wId of winningPlayerIds) {
            let bal = await (0, database_1.getBalance)(wId);
            bal += winShare;
            await (0, database_1.updateBalance)(wId, bal);
        }
        ...
        const winMsg = winningPlayerIds.map(id => `<@${id}>`).join(", ") + ` nhận **${(0, utils_1.formatMoney)(winShare)}** từ hũ cược!`;
        const showdownEmbed = new discord_js_1.EmbedBuilder()
    ...
    // Hẹn giờ lượt đi 60s
    const startTurnTimer = (activePlayer) => {
        if (turnTimeoutTimer)
            clearTimeout(turnTimeoutTimer);
