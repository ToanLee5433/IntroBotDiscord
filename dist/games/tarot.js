...
exports.initTarot = initTarot;
exports.handleTarot = handleTarot;
...
async function initTarot() {
    ...
    let found = 0;
    const missing = [];
    for (const card of exports.TAROT_DECK) {
    ...
    if (found === exports.TAROT_DECK.length) {
        console.log(`[TAROT] ✅ Thư viện ảnh đầy đủ ${found}/${exports.TAROT_DECK.length} lá — sẵn sàng!`);
    }
    else {
 ...
 */
async function handleTarot(message, rawInput) {
    ...
    if (balance < cost) {
        await message.reply(`❌ **Đéo đủ tiền xem bói!** Lệ phí cúng thầy Toàn là **${(0, utils_1.formatMoney)(cost)}**.\nVí mày chỉ có **${(0, utils_1.formatMoney)(balance)}**, cút đi cày cuốc rồi quay lại! 💸`).catch(() => { });
        return;
    }
    const now = Date.now();
    ...
    const hasTarot = await (0, database_1.hasTarotToday)(userId, todayStr);
    if (hasTarot) {
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('tarot_love').setLabel('💕 Tình Duyên').setStyle(discord_js_1.ButtonStyle.Danger), new discord_js_1.ButtonBuilder().setCustomId('tarot_career').setLabel('💼 Sự Nghiệp').setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId('tarot_money').setLabel('💰 Tiền Bạc').setStyle(discord_js_1.ButtonStyle.Success));
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle("🔮 BÓI BÀI TAROT — THẦY TOÀN GIANG HỒ")
        .setDescription(`Chào **${profile.name}**, vũ trụ đang lắng nghe mày.\nLệ phí cúng thầy Toàn là **${(0, utils_1.formatMoney)(cost)}**.\n\n👇 Hãy chọn chủ đề mày muốn bói dưới đây để thầy bắt đầu gieo quẻ:`)
        .setColor(0x9B59B6)
    ...
    collector.on('collect', async (i) => {
        ...
        try {
            ...
            let curBal = await (0, database_1.getBalance)(userId);
            if (curBal < cost) {
                await i.update({
                    content: `❌ **Đột nhiên nghèo đi à?** Lệ phí bói bài là **${(0, utils_1.formatMoney)(cost)}** nhưng ví mày hiện tại chỉ có **${(0, utils_1.formatMoney)(curBal)}**!`,
                    embeds: [],
                    components: []
                }).catch(() => { });
                return;
            ...
            // Thực hiện trừ tiền
            curBal -= cost;
            await (0, database_1.updateBalance)(userId, curBal);
            // Lưu ngày bói Tarot và cập nhật streak
            const streak = await (0, database_1.recordTarotPlay)(userId, todayStr, now);
            ...
            while (indexes.length < 3) {
                const rand = (0, utils_1.trueRandom)(0, 21);
                if (!indexes.includes(rand))
                    indexes.push(rand);
            }
            const cards = [exports.TAROT_DECK[indexes[0]], exports.TAROT_DECK[indexes[1]], exports.TAROT_DECK[indexes[2]]];
            const orients = cards.map(() => (0, utils_1.trueRandom)(1, 2) === 1 ? 'Xuôi ⬆️' : 'Ngược ⬇️');
...
`;
            let explanation = '';
            try {
                explanation = await (0, gemini_1.getTarotReading)(geminiPrompt);
            }
            catch (err) {
            ...
            let embedColor;
            if (hasDanger)
                embedColor = 0x7C0A02; // Đỏ máu — hung
            else if (reversedCount >= 2)
                embedColor = 0xE67E22; // Cam — cản trở
            else if (hasPositive)
                embedColor = 0xF1C40F; // Vàng — tốt lành
            else
                embedColor = 0x9B59B6; // Tím — bình thường
            ...
            cards.forEach((c, index) => {
                ...
                if (fs.existsSync(p))
                    attachments.push(new discord_js_1.AttachmentBuilder(p, { name: imgNames[index] }));
            });
            // Dựng 4 Embeds
            const buildCardEmbed = (cardIdx, posLabel, posIcon) => {
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .setColor(embedColor)
                    .setDescription(`**🃏 ${c.name}** *(${c.englishName})* — Hướng: ${o}\n` +
            ...
            const embedSummary = new discord_js_1.EmbedBuilder()
