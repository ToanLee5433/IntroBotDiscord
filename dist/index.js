...
const userVoiceCooldown = new Map();
client.on('voiceStateUpdate', async (oldState, newState) => {
    try {
        ...
        const resolveAudioFileForUser = (uId) => {
...
(async () => {
    try {
        ...
        fetch('https://api.ipify.org?format=json', { signal: AbortSignal.timeout(5000) })
            .then(r => r.json())
            .then((d) => console.log(`[DB CHẨN ĐOÁN] IP đầu ra của Service: ${d.ip}`))
        ...
        dbConnectionPromise = (0, database_1.connectDB)()
            ...
            if (client.isReady()) {
                await (0, warmup_1.loadWarmupVideosCache)(client).catch(err => console.error("Lỗi nạp cache video warmup sau khi kết nối DB:", err));
            }
        })
            .catch(err => {
