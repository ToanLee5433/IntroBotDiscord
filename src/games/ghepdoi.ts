import { 
    Message, EmbedBuilder, PermissionFlagsBits, UserSelectMenuBuilder, 
...
export function translateNaYin(naYin: string): string {
    ...
    for (const [zh, vi] of Object.entries(map)) {
        if (naYin.includes(zh)) return vi;
    }
    return naYin;
}
...
export function isValidDate(day: number, month: number, year: number): boolean {
    ...
    if (birthDate.getTime() > Date.now()) return false;
...
export function getWesternZodiacInfo(day: number, month: number) {
...
export function getLunarCompatibility(shengXiao: string) {
    const map: { [key: string]: { animal: string; emoji: string; tamHop: string; lucHop: string; xungChinhDien: string; xungNhom: string } } = {
...
export function getGangTitle(balance: number, debt: number, isPrincess: boolean = false): string {
    if (isPrincess) return '👑 CÔNG CHÚA GIANG HỒ';
    if (debt >= 100000) return '💸 CON NỢ QUỐC DÂN';
    if (balance >= 500000) return '🏦 ĐẠI GIA SERVER';
    if (balance >= 200000) return '💎 TRÙM SÒNG BÀI';
    if (balance < 10000) return '🍚 CÁI BANG TỰ NỢ';
    return '⚔️ DÂN CHƠI GIANG HỒ';
}

// ================= CANVAS PROFILE CARD GENERATOR =================

export async function generateProfileCardCanvas(
    targetUser: User, 
    profile: any, 
    balance: number, 
    debt: number, 
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(40, 95); ctx.lineTo(width - 40, 95); ctx.stroke();
    ...
    if (!isNaN(day) && !isNaN(month) && !isNaN(year)) {
        ...
        try {
            ...
            if (naYinRaw) menhText = translateNaYin(naYinRaw);
        } catch (e) {}
    }
...
const profileCooldowns = new Map<string, number>();

export function setupProfileInteractions(client: any) {
    client.on('interactionCreate', async (interaction: any) => {
        ...
        const debt = await getDebt(targetUserId);
        ...
        if (action === 'card') {
            ...
            try {
                ...
                let topBreakdown: FengShuiBreakdown | null = null;
                let sameBreakdown: FengShuiBreakdown | null = null;

                if (interaction.guild && profile && profile.birthday && profile.gender) {
                    const dateStr = getVNDateString(Date.now());
                ...
                let matchMsg = 
                    `💖 **KẾT QUẢ GHÉP ĐÔI NHANH CHUẨN XÁC VÀ NHẤT QUÁN** 💖\n\n` +
            ...
            await interaction.reply({
                content: `🏦 **Tài chính Ngân hàng của <@${targetUserId}>:**\n- 💵 **Ví tiền:** \`${formatMoney(balance)}\`đ\n- 💸 **Nợ ngân hàng:** \`${formatMoney(debt)}\`đ`,
                flags: MessageFlags.Ephemeral
            }).catch(() => {});
        }
    });
...
export function getCungPhi(birthdayStr: string, gender: string): CungPhiResult {
    ...
    if (gender === 'Nam') {
        ...
        if (quaiNum === 10) quaiNum = 1;
        if (quaiNum === 5) quaiNum = 2; // Nam 5 quy về Khôn (2)
    } else {
        quaiNum = 4 + remainder;
        while (quaiNum > 9) {
            quaiNum = Math.floor(quaiNum / 10) + (quaiNum % 10);
        }
        if (quaiNum === 5) quaiNum = 8; // Nữ 5 quy về Cấn (8)
    }

    const cungNames: { [key: number]: string } = {
        1: 'Khảm',
        2: 'Khôn',
        3: 'Chấn',
        4: 'Tốn',
...
export function getBatTrachRelation(cungA: string, cungB: string): BatTrachResult {
    const matrix: { [key: string]: { [key: string]: BatTrachResult } } = {
        'Càn': {
...
export function getFengShuiScore(zodiacA: string, zodiacB: string, menhA: string, menhB: string, cungA: string, cungB: string): number {
    ...
    const getElement = (menh: string): string => {
    ...
    score += trueRandom(-10, 10);

    return Math.max(0, Math.min(100, score));
}

export interface FengShuiBreakdown {
    totalScore: number;
    details: string[];
    verdict: string;
}

export function calculateFengShuiBreakdown(
    birthdayA: string, genderA: string,
    birthdayB: string, genderB: string
): FengShuiBreakdown {
    ...
    const solarA = Solar.fromYmd(yearA, monthA, dayA);
    ...
    const solarB = Solar.fromYmd(yearB, monthB, dayB);
    ...
    if (elA && elB) {
        if (elA === elB) {
            ...
            const sinhMap: { [key: string]: string } = {
            ...
            if (sinhMap[elA] === elB) {
                ...
                const khacMap: { [key: string]: string } = {
...
function getLoveBar(percent: number): { bar: string, status: string } {
    const totalSlots = 10;
...
export async function handleProfileRegistration(message: Message, rawInput: string) {
    ...
    if (!dobMatch) {
        ...
        if (profile && profile.name && profile.gender && profile.birthday) {
            ...
            const whoCrushedMeList = await getWhoCrushedMe(targetUser.id);
            ...
            embed.addFields(
                { name: "💘 Chỉ số Đào Hoa", value: `\`${crushCount}\` người đang âm thầm crush bí mật`, inline: true },
...
export async function playMatchmaking(message: Message) {
    ...
    const balanceA = await getBalance(userIdA);
    const debtA = await getDebt(userIdA);
    const balanceB = await getBalance(userIdB);
    const debtB = await getDebt(userIdB);
        ...
        if (failedMatches >= 5) {
            ...
            if (memberA) {
                ...
                if (!currentNick.includes("[🤡 Simp Lỏ]")) {
                    ...
                    if (nickChanged) {
                        await (message.channel as any).send(`🤡 **PHONG HIỆU CHÚA TỂ SIMP LỎ!** <@${userIdA}> đã ghép đôi thất bại 5 lần liên tiếp trong ngày với điểm dưới 20%. Đúng là gương mặt vàng trong làng ế chỏng vó, nhận phong hiệu **[🤡 Simp Lỏ]** khóa đầu 24 giờ cho tỉnh ngộ! 🤡`).catch(()=>{});
                    }
                }
            }
        }
    ...
    if (isJailTriggered && message.guild) {
        setTimeout(async () => {
            await banChat(userIdA, 120000);
            await banChat(userIdB, 120000);
            await sendToJail(message.guild!, userIdA, "Hợp nhau 100% - Đi tù chung hưởng tuần trăng mật");
            await sendToJail(message.guild!, userIdB, "Hợp nhau 100% - Đi tù chung hưởng tuần trăng mật");
        }, 3000);
    }
}
...
export async function handleDetectiveServices(message: Message, type: 'thamtu' | 'bandung') {
    ...
    if (type === 'thamtu') {
        const cost = 50;
        if (balance < cost) {
            await message.reply(`❌ **Nghèo rớt mồng tơi mà đòi thuê thám tử tư?** Phí dịch vụ thám tử tìm kiếm người thầm thương trộm nhớ mày là **${formatMoney(cost)}**! Lo cày cuốc hoặc vay ngân hàng đi cưng! 💸`).catch(()=>{});
            return;
        }
        ...
        const admirers = await getWhoCrushedMe(userId);
        const count = admirers.length;

        if (count === 0) {
            await message.reply(`🕵️‍♂️ **Thám tử tư BotToan báo cáo:**\nCầm **${formatMoney(cost)}** của mày tao đi dò la khắp hang cùng ngõ hẻm rồi. Kết quả là **ĐÉO CÓ AI** thầm thích mày cả! Vừa ế chỏng chơ vừa mất tiền ngu, chừa thói tò mò nhé con giời! 🤡💸`).catch(()=>{});
        } else {
        ...
        if (balance < cost) {
            await message.reply(`❌ **Đéo đủ tiền mua tin mật!** Dịch vụ bán đứng đồng bọn có giá **${formatMoney(cost)}**. Mày chỉ còn **${formatMoney(balance)}**, kiếm thêm tiền cúng tao đi! 💸`).catch(()=>{});
            return;
        }
...
export async function handleBuaYeu(message: Message) {
    ...
    const cost = 500;
    let balanceA = await getBalance(userIdA);
    if (balanceA < cost) {
        await message.reply(`❌ **Đéo đủ tiền mua bùa!** Bùa yêu ép duyên cưỡng chế có giá cực chát là **${formatMoney(cost)}**. Ví mày chỉ có **${formatMoney(balanceA)}**, nghèo thì chịu ế đi con! 💸`).catch(()=>{});
        return;
    }
