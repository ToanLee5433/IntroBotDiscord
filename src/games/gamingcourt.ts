...
import { generateGamingCourtVerdict } from '../services/gemini';
 ...
 */
export async function handleGamingCourt(message: Message, rawInput: string) {
    if (!message.guild) {
    ...
    let gameActivity = activities.find(act => act.type === ActivityType.Playing);
    if (!gameActivity) {
        gameActivity = activities.find(act => act.type === ActivityType.Streaming);
    }
    if (!gameActivity) {
        gameActivity = activities.find(act => act.type === ActivityType.Competing);
    }
    ...
    if (isMaVungKinhKin) {
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `🎮 **Tội trạng thực tế:** Nghiện game **${gameName}**\n` +
                `⏱️ **Thời gian phạm tội:** Đã cắm mặt vào game **${durationText}** liên tục.`
            )
            .setColor(0x1F1F1F) // Màu đen tối tăm rùng rợn
            .setThumbnail(targetUser.displayAvatarURL())
            .addFields(
                { name: "🏷️ TỘI DANH: HỆ TIẾN HÓA THÀNH MA VÙNG KÊNH KÍN ☠️", value: `Mày cày game hơn 8 tiếng rồi đấy ${pronoun.toLowerCase()} điên này! Thần linh cũng phải lạy mày, định chơi đến lúc hóa thạch luôn à? Đầu óc chắc nát map luôn rồi!` },
                { name: "⚖️ Phán quyết cuối cùng", value: "Cưỡng chế ngắt nguồn máy tính, tịch thu ổ cứng và xua đuổi lên giường đi ngủ ngay lập tức!" },
                { name: "📊 Chỉ số báo thủ", value: `\`[████████████████████]\` **100%** *(Hết cứu, báo nhà báo cửa báo cả dòng họ)*` }
            )
    ...
    // --- GỌI GEMINI LUẬN TỘI ---
    try {
        const geminiOutput = await generateGamingCourtVerdict(targetMember.displayName, gameName, durationText, pronoun);
        ...
        const embed = new EmbedBuilder()
            ...
            .setThumbnail(targetUser.displayAvatarURL())
            .addFields(
                { name: "📊 Chỉ số báo thủ", value: `\`[${bar}]\` **${percentage}%**` }
            )
