    ...
    if (['blackjack', 'xi dach', 'xidach', 'choi blackjack'].some(t => cleanInput.includes(t))) {
        ...
        const resolveDealerTurn = async (interaction: any) => {
            ...
            for (const pId in bjPlayers) {
                ...
                let wonAmt = 0; let status = "";

                if (p.state === 'busted') {
                    status = `💸 Bị Quắc - Mất ${p.bet}k`;
                } else if (p.state === 'blackjack') {
                    if (dealerBJ) { playerBalances[pId] += p.bet; status = `⚖️ Hòa Blackjack - Hoàn ${p.bet}k`; } 
                    else {
                        wonAmt = p.bet + (p.bet * 1.5); 
                        playerBalances[pId] += wonAmt;
                        status = `🤑 Xì Dách! Ăn lãi ${(p.bet * 1.5)}k`;
                    }
                } else {
                    if (dealerBust) { wonAmt = p.bet * 2; playerBalances[pId] += wonAmt; status = `🤑 Cái Quắc - Ăn lãi ${p.bet}k`; } 
                    else if (p.value > dealerVal) { wonAmt = p.bet * 2; playerBalances[pId] += wonAmt; status = `🤑 Thắng điểm - Ăn lãi ${p.bet}k`; } 
                    else if (p.value === dealerVal) { playerBalances[pId] += p.bet; status = `⚖️ Hòa điểm - Hoàn ${p.bet}k`; } 
                    else status = `💸 Thua điểm - Mất ${p.bet}k`;
                }
